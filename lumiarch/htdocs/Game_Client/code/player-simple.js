...
import { initMobileControls } from './mobile.js'; // Import mobile handler
...
export async function initPlayer(scene, world, spawnPos = { x: 0, y: 100, z: 0 }) {
    ...
    const contact = new CANNON.ContactMaterial(playerMat, playerMat, { friction: 0.0, restitution: 0.0 });
    ...
    // Auto-init mobile controls if on a touch device
    initMobileControls();
...
async function loadCharacterModel() {
    try {
        ...
        const shirtID = avatarData?.shirt_id || "3";
            ...
            itemList.forEach(item => {
                if (item.class === "Part" && item.properties.Name === "Torso") {
                    ...
                    item.children.push({
                        ...
                        properties: {
                            Name: "T-Shirt",
                            TexturePath: `../avatar/catalog/${shirtID}.png`,
                            Transparency: "0",
                            Side: "front",
                            Color3: "255, 255, 255",
                            ZIndex: "1"
        ...
        const offsets = {
            "Head":     { x: 0,    y: 1.5,  z: 0 },
            "Torso":    { x: 0,    y: 0,    z: 0 },
            "LeftArm":  { x: -1.5, y: 0,    z: 0 },
            "RightArm": { x: 1.5,  y: 0,    z: 0 },
            "LeftLeg":  { x: -0.5, y: -2.0, z: 0 },
            "RightLeg": { x: 0.5,  y: -2.0, z: 0 }
        };
...
export function updatePlayer(camera, world) {
    if (!initialized || !characterBody) return;

    const rayResult = new CANNON.RaycastResult();
    const rayFrom = characterBody.position;
    const rayTo = new CANNON.Vec3(rayFrom.x, rayFrom.y - (humanoid.hipHeight + 2), rayFrom.z);
    
    world.raycastClosest(rayFrom, rayTo, { skipBackfaces: true }, rayResult);

    let isGrounded = false;
    if (rayResult.hasHit && characterBody.velocity.y <= 1.1) {
        if (rayResult.distance <= humanoid.hipHeight + 0.1) {
            isGrounded = true;
            humanoid.state = "Grounded";
            characterBody.position.y = rayResult.hitPointWorld.y + humanoid.hipHeight;
    ...
    if (moveDir.lengthSq() > 0) {
        ...
        if (isGrounded) {
            characterBody.velocity.x = targetX;
            characterBody.velocity.z = targetZ;
        } else {
            characterBody.velocity.x += (targetX - characterBody.velocity.x) * 0.1;
            characterBody.velocity.z += (targetZ - characterBody.velocity.z) * 0.1;
        }
        ...
        characterBody.velocity.x *= 0.1;
        characterBody.velocity.z *= 0.1;
    }
