...
function onWindowResize() {
    ...
    const leftWidth = 250;   // your left sidebar
    const rightWidth = 300;  // your right properties panel
    const topHeight = 115;   // top bar
    const bottomHeight = 150; // bottom bar/toolbox/etc.
...
function createBrick(x, y, z, sx, sy, sz, rx, ry, rz, mass, anchored, materials, partType) {
    ...
    // Apply Rotation to Cannon Body (Euler to Quaternion)
    ...
    // Apply Rotation to Three.js Mesh
    mesh.rotation.set(radX, radY, radZ);

    body.meshIndex = meshes.length; // Fixed index logic
