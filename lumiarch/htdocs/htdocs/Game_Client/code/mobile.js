...
export function initMobileControls() {
    ...
    overlay.style.cssText = `
        ...
        background: rgba(0,0,0,0.95); color: white; z-index: 1000000;
    ...
    // Attempt to lock orientation (requires a user gesture usually, or fullscreen)
    const tryLockOrientation = () => {
        if (screen.orientation && screen.orientation.lock) {
            screen.orientation.lock('landscape').catch(() => {
                console.log("Orientation lock requires Fullscreen or is not supported.");
            });
        }
    };
    ...
    // Try to lock when they first touch the screen
    ...
    window.addEventListener('touchstart', (e) => {
        ...
        const isGUI = touch.target === leftZone || leftZone.contains(touch.target) || 
                      touch.target === jumpBtn || touch.target.closest('#orientation-overlay');
