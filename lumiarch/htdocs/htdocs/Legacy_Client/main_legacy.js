    ...
    items.forEach(item => {
        ...
        if (className === "Workspace") {
            if (properties) {
                ...
                if (grav) {
                    // Update global Workspace variable
                    Workspace.gravity = -parseFloat(grav.getAttribute('value'));
                    world.gravity.set(0, Workspace.gravity, 0);
                }
            }
            // Recurse to find Parts inside Workspace
            processElements(item);
        } 
        
        else if (className === "Part") {
            parseObject(item); // Using the updated version below
        }
...
function parseObject(itemNode) {
    ...
    const partType = props.querySelector('partType').textContent;
    
    // Position, Size, and Rotation use uppercase X, Y, Z in your new XML
    const posNode = props.querySelector('position');
    const sizeNode = props.querySelector('size');
    const colorHex = props.querySelector('brickcolor').textContent.replace('#', '0x');
    ...
    const anchored = props.querySelector('anchored').textContent === 'true';
    const canCollide = props.querySelector('cancollide').textContent !== 'false';
...
function createBrick(x, y, z, sx, sy, sz, mass, anchored, materials, partType) {
    ...
    body.meshIndex = meshes.length - 1;
